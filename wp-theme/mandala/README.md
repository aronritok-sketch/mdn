...
| `mailer.php`, `automations.php` | **Levélközpont:** a téma minden automata levele egy helyen (elhagyott kosár, használati útmutató, értékelés kérése, újrarendelés, újra raktáron, ajándékutalvány, feladás, átvehető, heti B2B) – szerkeszthető tárgy / címsor / szöveg helyőrzőkkel (`{keresztnev}`, `{rendeles}`…) és tartalomblokkokkal (`{termekek}`, `{gomb}`…), be/ki és időzítés, élő előnézet mintaadatokkal, tesztlevél, alaphelyzet, aláírás, válaszcím; napló 180 napig (keresés e-mailre / rendelésre, a rendelés oldalán is); **a WooCommerce saját levelei is itt szerkeszthetők** (be/ki, tárgy, címsor, kiegészítő szöveg, előnézet, tesztküldés a legutóbbi rendeléssel) és naplózódnak; leiratkozás, `List-Unsubscribe`. **Saját levelek** (`mailer-custom.php`): trigger (új rendelés, fizetés, bármely állapot, feladva, átvehető, X nap az utolsó rendelés óta, regisztráció, hírlevél, értékelés) + késleltetés + feltételek (kategória, cikkszám, min. összeg, szállítás, fizetés, első / visszatérő, viszonteladó, ország) + címzett (vásárló / bolt / megadott címek) + opcionális egyedi, egyszer használható kupon; rendelésenként egyszer, küldéskor újraellenőrizve. WPML: a saját szövegek fordítható stringek (`mandala-mail`) | WooCommerce → Mandala levelek |
